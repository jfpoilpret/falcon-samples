...
def init_teams(session):
	# type: (Session) -> None
	with io.open('example5/data/teams.txt') as f:
		for line in f:
			fields = line[:-1].split('\t')
...
def init_venues(session):
	# type: (Session) -> None
	with io.open('example5/data/venues.txt') as f:
		for line in f:
			name = line[:-1]
...
def init_users(session):
	# type: (Session) -> None
	with io.open('example5/data/users.txt') as f:
		for line in f:
			fields = line[:-1].split('\t')
...
def init_matches(session, tz):
	# type: (Session, str) -> None
	with io.open('example5/data/matches.txt') as f:
		num = 1
		for line in f:
			fields = line[:-1].split('\t')
